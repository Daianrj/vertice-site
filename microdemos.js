...
erp:(s=0)=>`<div class="microapp micro-erp"><aside><strong>V/ ERP</strong><span class="${s===0?'active':''}">Dashboard</span><span>Clientes</span><span>Produtos</span><span class="${s===2?'active':''}">Estoque</span><span class="${s===1?'active':''}">Pedidos</span><span>Entregas</span></aside><main>
<div class="micro-kpis"><b>128<small>Clientes</small></b><b>${s?33:32}<small>Pedidos hoje</small></b><b>R$ 18,4k<small>Faturamento</small></b><b class="${s===2?'ok':''}">${s===2?5:6}<small>Pendências</small></b></div>
<div class="micro-erp-event"><strong>${s===0?'Visão geral':s===1?'Pedido PED-8841 criado': 'Estoque reservado ✓'}</strong><span>${s===0?'Operação centralizada':s===1?'Mercado Horizonte · 8 volumes':'Pedido liberado · Dashboard atualizado'}</span></div>
<div class="micro-chart"><i style="height:42%"></i><i style="height:64%"></i><i style="height:${s===2?'88%':'72%'}"></i><i style="height:58%"></i></div></main></div>`,
...
routing:(s=0)=>`<div class="microapp micro-route"><div class="micro-route-head"><strong>Carlos Souza · Oeste 01</strong><span>3 paradas · 32,4 km</span></div>${['Bangu · NF 45821','Campo Grande · NF 45834','Santa Cruz · NF 45842'].map((x,i)=>`<div class="micro-route-stop ${i===s?'active':''}"><b>0${i+1}</b><span>${x}</span><em>${i<s?'Concluída':i===s?'Próxima':'Pendente'}</em></div>`).join('')}</div>`,
...
'sheet-system':(s=0)=>`<div class="microapp micro-sheet-system" data-stage="${s}"><div class="sheet-before"><strong>CLIENTES.xlsx</strong><div>Nome · Telefone · Pedido · Status · Valor</div><i></i><i></i><i></i><i></i></div><div class="sheet-pipeline">${['Planilha','Automação','Dados','Web App','Dashboard','Sistema'].map((x,i)=>`<b class="${i<=s?'on':''}">${x}</b>`).join('<span>→</span>')}</div><div class="sheet-after"><strong>${s<3?'Processos manuais':'Sistema operacional'}</strong><span>${s<3?'dados dispersos · retrabalho':'automação · dashboard · controle'}</span></div></div>`
};
